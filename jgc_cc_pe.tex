...
nodes. {\color{red} There are a plethora of static WCRT estimation
  ...
  GRC to find the WCRT of the SystemJ clock-domain. The exhaustive
  search space carried out by the Uppaal model-checker gives WCRT
...
{\color{red} The algorithm (Figure~\ref{fig:11b}) used to replace the
  ...
  \texttt{amultinewarray} bytecode takes 4 bytes in the class file. The
  \texttt{new} and \texttt{anewarray} bytecodes are encoded in the class
  file with three bytes, whereas the \texttt{newarray} bytecode only
  uses two bytes in the class file representation.  
