...
\begin{abstract}
  ...
  {\color{red} Programs written in synchronous and \textit{Globally
      Synchronous Locally Synchronous} (GALS) programming languages are
    amenable to formal verification and \textit{Worst Case Execution
      Time} (WCET) analysis, since they are based on the formal finite
    state machine \textit{Model of Computation} (MoC). Property
    verification and static WCET analysis makes programs developed in
    these paradigms suitable for designing safety critical hard
    real-time systems. SystemJ is a programming language based on the
    GALS MoC. SystemJ uses the Java programming language as the ``host''
    ...
    The same GC, which helps in functional verification, makes WCET
    analysis extremely challenging. Any WCET analysis framework for
    synchronous and GALS programs needs to consider the operations
    performed by the host language. Recently it has been shown that the
    static WCET estimates of programs invoking GC are highly pessimistic
    (in seconds, whereas the program's WCET itself is in
    micro-seconds). These pessimistic estimates renders the WCET
    analysis framework ineffective. In order to overcome this problem,
    we develop a compiler assisted memory management technique for
    applications in SystemJ. The SystemJ MoC, clearly demarcates the
    state boundaries of the program, which in turn allows us to
    partition the heap, at compile time, into two distinct areas: (1)
    the memory area called the \textit{permanent heap} that holds
    objects that are live throughout the life time of the application,
    and (2) the memory area used to hold all other objects, called the
    \textit{transient heap}. Both the memory areas are bounded
    statically at compile time. Furthermore, the memory allocation and
    reclaim procedures are simple stack load and pointer reset
    operations, respectively, which are guaranteed to complete within a
    bounded number of clock-cycles, thereby alleviating the need for
    large pessimistic WCET bounds obtained due to the GC. Other than
    being amenable to formal verification and tight WCET analysis,
    results show that our proposed approach to memory management is
    approximately three times faster, in terms of memory allocation,
...
\section{Static WCRT analysis of of SystemJ clock-domains}
...
{\color{red} physical} memory address, the program points to replace and
...
paper. {\color{red} A more through comparison between the proposed
