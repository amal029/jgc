...
\begin{abstract}
  ...
  shown that the WCET estimates for garbage collection cycles are very
  pessimistic (in seconds, whereas the program's WCET itself is in
  micro-seconds). These pessimistic estimates renders the WCET analysis
  framework ineffective. In order to overcome this problem, we develop a
  compiler assisted memory management technique for applications written
  in SystemJ. {\color{red} The SystemJ MoC plays the central role in the
...
{\color{red}Once emitted, the status of a signal is \texttt{true} only
...
{\color{red} Finally, there are two types of loops in SystemJ: temporal
  loops consisting at least one $\mathbf{pause}$ statement, that execute
  forever and bounded data-loops like in standard Java. }
...
\begin{definition} \textit{Clock-domain transition (or tick)}: A single
  ...
  with no incoming edges, e.g., node S1 in Figure~\ref{fig:2a}) to the
  sink node (node with no outgoing edges, e.g., node J1 in
  Figure~\ref{fig:2a}).
\end{definition}
...
organizations two \underline{requirements} become very important:
...
\item \underline{Step6} can be conceptually partitioned into two parts.
  ...
  method that the field is being used. {\color{red} The more interesting
...
slower than the proposed approach. {\color{red} Close analysis of the
...
{\color{red}
  ...
  The work in~\cite{gay1998memory} propose a region based memory
...
another tasks scoped memory) in SCJ. This is because, SCJ allows context
switching of tasks during program execution, whereas in SystemJ a
clock-domain tick is atomic. RTSJ does allow access of scoped memory by
concurrent threads and only when the last thread exits, the scoped
...
language runtimes, primarily Java. {\color{red} The SystemJ model of
...
non-pessimistic worst case execution time analysis. {\color{red} Java
