...
\begin{abstract}
  ...
  verification, makes \textit{Worst Case Execution Time} (WCET) analysis
  challenging. Any WCET analysis framework for GALS programs needs to
  consider the operations performed by the host language. It has been
  shown that the WCET estimates for garbage collection cycles are in
  seconds, whereas the program's WCET itself is in micro-seconds. These
  pessimistic estimates renders the WCET analysis framework ineffective.
  In order to overcome this problem, we develop a compiler assisted
  memory management technique for applications written in SystemJ.
  {\color{black} The SystemJ MoC plays the central role in the proposed
    technique. The SystemJ MoC allows clearly demarcating the state
    boundaries of the program, which in turn allows us to partition the
    heap, at compile time, into two distinct areas: (1) the memory area
    called the \textit{permanent heap}, which holds objects that are
    alive throughout the life time of the application, and (2) the
    memory area used to hold all other objects, called the
  ...
  thereby alleviating the need for large pessimistic WCET bounds
...
\item \underline{Step5} extracts the variable from the affected
  ...
  execution. For each of these program points, {\color{black} Step5
...
We chose 5 real-time synchronous programs for bench
marking. \texttt{Cruise control}~\cite{charles1996representation} is the
cruise speed controller found in
