...
and \texttt{newarray} bytecodes. Moreover, there are two types of array
allocation bytecodes, dependent upon the type of the array being
allocated: (1) the \texttt{newarray} bytecode that allocates space for
arrays holding primitive values and (2) the \texttt{anewarray} bytecode
that allocates space for arrays holding object references. This
distinction is important since the number of bytes used to represent
each of these bytecode varies. The \texttt{new} and \texttt{anewarray}
bytecodes are encoded in the class file with three bytes, whereas the
\texttt{newarray} bytecode only uses two bytes in the class file
representation.
...
implementations~\cite{msch05,ipat13} and (2) The \textit{Safety Critical
  Java} (SCJ)~\cite{scj2013} implementation of Java on the JOP
platform. The first RTGC framework is the one proposed in~\cite{msch05},
which is a concurrent version of Cheney's copy collector developed by
...
implementation of the allocation and real-time collector described
in~\cite{pizlo2010schism}, in the rest of the section we term it AGC. In
case of SCJ, all the micro-benchmarks were implemented as single tasks.
All our micro-benchmark experiments are run in ModelSim, simulating the
real-time hardware implemented JVM called JOP, running at 100 MHz, with
...
in \texttt{NullPointer} exceptions. There is a greater speedup in case
of array allocations compared to simple object allocations. The runtime
memory allocation time difference is significant in the case of GC
invocation when compared to the case of no GC invocation. 
...
time is far larger than all the other memory allocation times. In
particular, the SCJ memory allocation time in these experiments is
$\approx 7.3 \times$ worse than the memory allocation time compared to
the NO-GC approach.  Close analysis of the SCJ implementation on the JOP
...
multiple reasons for the speedup in the No-GC case: (1) there are no
read/write barriers when allocating memory, which are needed by both:
JOP-GC and AGC. (2) The current \texttt{new}, \texttt{newarray}, and
\texttt{anewarray} bytecodes are implemented in software, which means,
on \textit{every} execution of these bytecodes, a method may be loaded
into the method cache, which results in increased program latency. These
runtime numbers are important, because they influence the WCRT as we
will see in the next section. (3) In case of the \texttt{newarray} and
\texttt{anewarray} bytecode execution, array spines need to be
initialized for AGC~\cite{pizlo2008study,ipat13}, which leads to further
slow down in memory allocation times.
...
\begin{table}[t!]
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    \hline
    1000 & 28 & 21450 & 279050 & 2145 \\
    \hline
    2000 & 28 & 42900 & 559 & 4290000 \\
    \hline
...
SCJ like the other GC approaches allocates memory for each \texttt{new}
bytecode execution. SCJ, unlike the proposed No-GC approach, is unable
to reuse memory space allocated in the scoped memory area during the
loop iterations. The scoped memory area can only be reused when the
task, using the scoped memory area, finishes execution. The No-GC
approach greatly benefits from the static analysis proposed in this
paper, even though the memory organization of the two approaches is
similar.
...
analysis. Puffitsch et al.~\citeyear{puffitsch2013design} have already
shown that GC cycle times are orders of magnitudes larger than the
execution times of real-time tasks. Explicitly deleting all GC related
code makes this a fair comparison. The JOP-GC WCRT results are not
shown, because JOP-GC always results in \texttt{NullPointer} exceptions
for all these examples. We also do not compare with SCJ, because: (1)
the open source JOP based SCJ implementation is na{\"i}ve resulting in
large slowdowns during memory allocations, as seen in
Figures~\ref{fig:12} and \ref{fig:16}. (2) All the real-time benchmark
programs are reactive, and run in an infinite loop allocating and
releasing memory. SCJ specification does not allow reuse of memory
words, in the scoped memory area, until the real-time task completes
execution (as seen in Table~\ref{tab:1}) and hence, all our real-time
benchmarks exhaust all memory when executed as SCJ tasks.
