...
internal data changes do not lead to change in the program state thereby
...
\item \textit{Real-time analyzable back-end code generation}: We present
  ...
  placement strategy, which allows for O(1) heap accesses in every case.
\end{itemize}
...
it and hence, there are only two program points: the \textit{invoke} and
the \textit{return} bytecodes that can lead to a method cache miss,
which can be accounted for in the static WCRT analysis procedure with
ease. Finally, the interface connecting the ReCOP and JOP has single
clock cycle latency as described in~\cite{DBLP:journals/tecs/SalcicM13}.
...
incremented on the LTS by the WCET value of the AGRC node encountered.
...
hence, all possible paths need to be explored to guarantee find a tight
...
events, but one still suffers from the problem of unbounded GC cycle
time, since a GC cycle needs to complete within the available slack.

Our solution to the above problem is to remove garbage collection
...
heap and the permanent heap. Looking at this new runtime memory
...
\item \underline{Step1} (Figure~\ref{fig:8a}) of the data-flow analysis
  ...
  initializing signals (e.g., method \texttt{init} in
  Figure~\ref{fig:8h}). A globally accessible multi-set $pp$
  (Figure~\ref{fig:8a}, line~\ref{algone:l1}) is defined that will hold
  all the program points initializing the objects that will be placed in
  ...
  initialization and places these program points into the multi-set
  $pp$. Each individual set within the multi-set $pp$ consists of a one
  or more program points that initialize objects. After the scan phase,
  ...
  second element is the initialization program point~\footnote{The
    initialization program point is actually at the bytecode level, but
    in this paper we keep the program points at the Java source code
    level for ease of understanding.} for the example in
...
\item \underline{Step3} (Figure~\ref{fig:8c}) analyzes the emission
  ...
  is being emitted (or affected) and hence, Step5 is called.
...
\item \underline{Step5} (Figure~\ref{fig:8e}) extracts the variable from
  ...
  Step3 is invoked for migrating these object allocations initializing
  the reference fields into permanent heap. Consider, the program
  point~\ref{exone:l2}, object \texttt{t} is initialized as class
  \texttt{c} (for program point~\ref{exone:l1}, \texttt{t} is
  ...
  execution due to polymorphism. We compute the \textit{instance} size
  of both these types as 3 and 4 words, respectively~\footnote{According
    to Java semantics, all fields of a parent class are inherited by the
  ...
  header size) in permanent heap space. Step7 returns multiple program
  points in set $defs$ \textrm{iff} these program points are mutually
  unreachable. This, guarantees that the reserved permanent heap space
  can be \textit{reused} at runtime by all program points in set $defs$.
  \newline
...
\item \underline{Step7} (Figure~\ref{fig:8g}), given a program point
  ...
  defining program points are themselves affecting variables, the Step7
  is called recursively to follow the so called deferred program
  edges~\cite{burke1995flow} to finally reach one or more program points
  that allocates the object or if the variable is not initialized then
  gives a compile time \texttt{not initialized} error
  (line~\ref{algseven:l3}). If the defining program point affects a
  field, then Step4 is called, which calls Step7 via Step6, we describe
  Step4 and Step6 next. If the defining program point is anything but
  any of the aforementioned statements, then an error is raised, stating
  that the variable might be initialized outside the method being
  analyzed (lines~\ref{algseven:l4}-\ref{algseven:l5}). Finally, Step7
  makes sure that the program points to return are not reachable from
  each other.\newline
  \begin{compactitem}[$\boxplus$]
  \item \textbf{Design decision}: We have consciously decided to not
    ...
    \texttt{new}/\texttt{newarray} bytecodes with a new set of bytecodes
    (c.f. Section~\ref{sec:prod-back-code}). We cannot blindly replace
    these bytecodes in methods other than the ones being analyzed,
    because, consequently \textit{any} other program point, related or
    unrelated to signals, using the same object initialization point
    would overwrite the object value as it inadvertently shares the same
    object. A simple example elucidating the situation is shown in
    Figure~\ref{fig:10}.
    ...
    is done for the object \texttt{B} in
...
\item \underline{Step6} (Figure~\ref{fig:8f}) can be conceptually
  ...
  allocating field \texttt{A}, this program point (line~\ref{extwo:l1})
  is returned by Step6. Note that multiple callers might call the same
  callee, due to \textit{may} happen before results produced by
...
need to be incremented by 22 bytes after replacement. This 22 byte shift
is made obvious by Figures~\ref{fig:11c} and~\ref{fig:11d}, which show
...
type (class) of object to allocate. The returned object reference is
then duplicated (\texttt{dup} instruction). Next, the constructor of the
class \texttt{a} is invoked to initialize the returned reference and
finally, the returned reference is put into a static (class) field of
the class containing the \texttt{main} method. The \texttt{new} bytecode
is timing \textit{unpredictable}, because it might invoke the GC, which
has an unbounded collection cycle. It might also generate a runtime out
of memory exception, thereby violating safety criticality. We replace
this \texttt{new} bytecode with timing predictable bytecodes as shown in
Figure~\ref{fig:11d}.

A single \texttt{new} bytecode (consuming 3 bytes in the class file,
hence the 22 byte increment for targets of conditional bytecodes) is
...
collector (RTGC) implementations~\cite{msch05,ipat13}. The first is the
RTGC proposed in~\cite{msch05}, which is a concurrent version of
Cheney's copy collector developed by Baker~\cite{baker1978list}. In the
rest of the section we call it JOP-GC. The second one is proposed
in~\cite{ipat13} is a variation of the implementation of the allocation
and collector described in~\cite{pizlo2010schism}, in the rest of the
section we term it AGC. All our micro-benchmark experiments are run in
...
bytecodes are implemented in software, which means, on every invocation
(loop iteration) a method needs to be loaded into the method cache,
which results in increased program latency. These runtime numbers are
important, because they influence the WCRT as we will see in the next
section. (3) In case of \texttt{newarray} bytecode execution, array
...
uses block based allocation, whereas JOP-GC object replication strategy
with semi-space~\footnote{In the numbers in Table~\ref{tab:1}, we have
  not included the semi-space reserved by JOP-GC.}.
...
Consequently this also has a penalty on the computed WCRT value, as the
method cache load time increases, but it is not as significant as
loading the methods implementing the object allocation for every
\texttt{new}/\texttt{newarray} bytecode execution in the AGC approach.
...
needed in different phases of the GC, and hence, preemptive and
incremental scheduling approaches have been the only solution to
programming real-time systems with managed runtime environments to
date. Preemptive scheduling generally leads to explosion in the number
of states that need to be checked by automated formal verification
tools, and hence, makes building safety critical systems hard, if not
impossible. This proposal provides a solution to this problem. All the
aforementioned improvements are further accompanied by improvements in
the program throughput (approximately $3\times$ faster memory allocation
times) and worst case times, as synchronization barriers necessary in
real-time garbage collection approaches become unnecessary in the
proposed approach. Not to mention the improvements in the number, size
and improved layout of allocated Java objects and arrays.
