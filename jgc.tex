...
\begin{figure}[t!]
  \centering
  \includegraphics[scale=0.7]{tpjop}
...
\item \underline{Step5} (Figure~\ref{fig:8e}) extracts the variable from
  ...
  checks if their are any reference fields within this object, if so,
...
\item \underline{Step6} (Figure~\ref{fig:8f}) can be conceptually
  ...
  initializing objects within the same method that the field is being
...
allocation time when no GC is invoked, there are two major reasons for
...
strategy with semi-space~\footnote{In the numbers in Table~\ref{tab:1},
  we have not included the semi-space reserved by JOP-GC.}.
...
preemptive and incremental scheduling approaches have been the only
solution to programming real-time systems with managed runtime
